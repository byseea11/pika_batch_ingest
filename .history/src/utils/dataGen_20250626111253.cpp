...
void DataGen::generateData()
{
    ...
    {
        pool.emplace_back(&DataGen::generateFile, this, perFileDataSize);
    }
    pool.emplace_back(&DataGen::generateFile, this, perFileDataSize + remainder);
...
std::string DataGen::generateKey()
{
    ...
    size_t index = dist(gen) % keyPool_.size(); // 此处仍需锁
